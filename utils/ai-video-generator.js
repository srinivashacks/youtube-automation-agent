...
class AIVideoGenerator {
  ...
  async generateVisualAssets(prompt, style = 'cognitive-angle', count = 1) {
    this.logger.info(`Generating ${count} character-aware visual asset(s)`);
    const localPaths = [];
  ...
  async generateImage(prompt, imagePath) {
    ...
    if (this.gemini) return this.generateGeminiImage(prompt, imagePath);
    if (this.openai) return this.generateOpenAIImage(prompt, imagePath);
    throw new Error('No image generation provider configured. Configure GEMINI_API_KEY or OPENAI_API_KEY.');
  }
  ...
  enhanceVisualPrompt(prompt, style) {
    return `${prompt}. Visual style: clean modern 2D educational illustration for the YouTube channel Cognitive Angle. Consistent recurring character, expressive but natural face, readable composition, cinematic 16:9 framing, subtle depth, polished digital illustration, no text, no watermark.`;
  }
  ...
  async generateThumbnail(script, style = 'cognitive-angle') {
    ...
    return { path: thumbnailPath, dimensions: { width: 1536, height: 1024 }, fileSize: await this.getFileSize(thumbnailPath) };
  }
